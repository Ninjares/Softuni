...
namespace Directory_Traversal
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var desktopreport = new StreamWriter(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + @"\DirReport.txt"))
            {
                foreach (var (extension, value) in dirInfo)
                {
                    desktopreport.WriteLine(extension);
                    foreach (var (filename, size) in value)
                    {
                        desktopreport.WriteLine($"--{filename} - {size:F3}kb");
                    }
